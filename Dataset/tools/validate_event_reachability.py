...
def validate_scene(scene_path: Path) -> list[str]:
    ...
    for action_type in (
        ...
        "remove_entity",
        "set_visual_state",
        "animate_entity_transform",
        "set_facility_state",
        "set_logical_boundary_state",
    ...
    missing_intent_events = [
        str(event.get("event_id") or event.get("topic") or index)
        for index, event in enumerate(script.get("events", []))
        if not event.get("intent") and not dict(event.get("payload") or {}).get("intent")
    ]
    if missing_intent_events:
        issues.append(f"{script['scenario_id']}: every event must declare explicit intent (missing={missing_intent_events})")
    ordered = sorted(
        (
    ...
    for _, topic in ordered:
        ...
        for row in interpreter.get_event_log():
            ...
            if row_topic == topic:
                ...
                break
    ok, matched_topics = required_intent_sequence_matches(contract.required_intents, ordered_rows)
    if not ok:
        issues.append(
            f"{script['scenario_id']}: required_intents order mismatch: {contract.required_intents} "
