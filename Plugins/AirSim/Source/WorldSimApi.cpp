...
namespace
{
FString MakeAeroBridgeUnavailableResponse(const TCHAR* OperationName)
{
    return FString::Printf(
        TEXT("{\"api_version\":\"1.0\",\"op\":\"%s\",\"status\":\"error\",\"payload\":{},\"error\":{\"message\":\"AeroBridgeWorldSubsystem is unavailable.\"}}"),
        OperationName);
}

const APIPCamera* ResolveDirectorCamera(const ASimModeBase* simmode, const std::string& camera_name)
{
    if (simmode == nullptr || simmode->CameraDirector == nullptr) {
        return nullptr;
    }

    FString name = FString(camera_name.c_str()).TrimStartAndEnd().ToLower();
    if (name == TEXT("external") || name == TEXT("externalcamera")) {
        return simmode->CameraDirector->getExternalCamera();
    }
    if (name == TEXT("backup") || name == TEXT("backupcamera")) {
        return simmode->CameraDirector->getBackupCamera();
    }
    if (name == TEXT("front") || name == TEXT("frontcamera")) {
        return simmode->CameraDirector->getFrontCamera();
    }
    if (name == TEXT("fpv") || name == TEXT("director_fpv")) {
        return simmode->CameraDirector->getFpvCamera();
    }

    return nullptr;
}

const APIPCamera* ResolveRpcCamera(const ASimModeBase* simmode, const WorldSimApi::CameraDetails& camera_details)
{
    if (camera_details.vehicle_name.empty()) {
        if (const APIPCamera* director_camera = ResolveDirectorCamera(simmode, camera_details.camera_name)) {
...
std::string WorldSimApi::invokeAeroBridge(const char* operation_name, const char* function_name, const std::string& request_json)
{
    const FString operation_name_text = UTF8_TO_TCHAR(operation_name);
    const FString function_name_text = UTF8_TO_TCHAR(function_name);
    FString response = MakeAeroBridgeUnavailableResponse(*operation_name_text);
    UAirBlueprintLib::RunCommandOnGameThread([this, &request_json, &response, operation_name_text, function_name_text]() {
        UWorld* world = simmode_ ? simmode_->GetWorld() : nullptr;
        UClass* bridge_class = LoadClass<UWorldSubsystem>(nullptr, TEXT("/Script/AeroBridgeRuntime.AeroBridgeWorldSubsystem"));
        UWorldSubsystem* bridge = (world != nullptr && bridge_class != nullptr) ? world->GetSubsystemBase(bridge_class) : nullptr;
        if (bridge == nullptr) {
            response = MakeAeroBridgeUnavailableResponse(*operation_name_text);
            return;
        }

        UFunction* bridge_function = bridge->FindFunction(FName(*function_name_text));
        if (bridge_function == nullptr) {
            response = FString::Printf(
                TEXT("{\"api_version\":\"1.0\",\"op\":\"%s\",\"status\":\"error\",\"payload\":{},\"error\":{\"message\":\"AeroBridgeWorldSubsystem is missing function %s.\"}}"),
                *operation_name_text,
                *function_name_text);
            return;
        }

        struct FAeroBridgeParams
        {
            FString RequestJson;
            FString ReturnValue;
        };

        FAeroBridgeParams params;
        params.RequestJson = FString(request_json.c_str());
        bridge->ProcessEvent(bridge_function, &params);
        response = params.ReturnValue;
    },
                                             true);
    return TCHAR_TO_UTF8(*response);
}

std::string WorldSimApi::simAeroDescribeCapabilities(const std::string& request_json)
{
    return invokeAeroBridge("simAeroDescribeCapabilities", "HandleDescribeCapabilities", request_json);
}

std::string WorldSimApi::simAeroLoadContext(const std::string& request_json)
{
    return invokeAeroBridge("simAeroLoadContext", "HandleLoadContext", request_json);
}

std::string WorldSimApi::simAeroReloadConfig(const std::string& request_json)
{
    return invokeAeroBridge("simAeroReloadConfig", "HandleReloadConfig", request_json);
}

std::string WorldSimApi::simAeroApplyFrame(const std::string& request_json)
{
    return invokeAeroBridge("simAeroApplyFrame", "HandleApplyFrame", request_json);
}

std::string WorldSimApi::simAeroPollFeedback(const std::string& request_json)
{
    return invokeAeroBridge("simAeroPollFeedback", "HandlePollFeedback", request_json);
}

std::string WorldSimApi::simAeroPedSpawn(const std::string& request_json)
{
    return invokeAeroBridge("simAeroPedSpawn", "HandlePedSpawn", request_json);
}

std::string WorldSimApi::simAeroPedReset(const std::string& request_json)
{
    return invokeAeroBridge("simAeroPedReset", "HandlePedReset", request_json);
}

std::string WorldSimApi::simAeroPedSetTarget(const std::string& request_json)
{
    return invokeAeroBridge("simAeroPedSetTarget", "HandlePedSetTarget", request_json);
}

std::string WorldSimApi::simAeroPedObserve(const std::string& request_json)
{
    return invokeAeroBridge("simAeroPedObserve", "HandlePedObserve", request_json);
}

std::string WorldSimApi::simAeroPedPlayAnimation(const std::string& request_json)
{
    return invokeAeroBridge("simAeroPedPlayAnimation", "HandlePedPlayAnimation", request_json);
}

std::string WorldSimApi::simAeroPedCommitCross(const std::string& request_json)
{
    return invokeAeroBridge("simAeroPedCommitCross", "HandlePedCommitCross", request_json);
}

std::string WorldSimApi::simAeroPedStop(const std::string& request_json)
{
    return invokeAeroBridge("simAeroPedStop", "HandlePedStop", request_json);
}

std::string WorldSimApi::simAeroPedSetVariant(const std::string& request_json)
{
    return invokeAeroBridge("simAeroPedSetVariant", "HandlePedSetVariant", request_json);
}

std::string WorldSimApi::simAeroPedRelease(const std::string& request_json)
{
    return invokeAeroBridge("simAeroPedRelease", "HandlePedRelease", request_json);
}

std::string WorldSimApi::simAeroPedSpawnCrowd(const std::string& request_json)
{
    return invokeAeroBridge("simAeroPedSpawnCrowd", "HandlePedSpawnCrowd", request_json);
}

std::string WorldSimApi::simAeroPedClearCrowd(const std::string& request_json)
{
    return invokeAeroBridge("simAeroPedClearCrowd", "HandlePedClearCrowd", request_json);
}

std::string WorldSimApi::simAeroPedRespawnCrowd(const std::string& request_json)
{
    return invokeAeroBridge("simAeroPedRespawnCrowd", "HandlePedRespawnCrowd", request_json);
}

std::string WorldSimApi::simAeroSpawnAsset(const std::string& request_json)
{
    return invokeAeroBridge("simAeroSpawnAsset", "HandleSpawnAsset", request_json);
}

std::string WorldSimApi::simAeroMoveAsset(const std::string& request_json)
{
    return invokeAeroBridge("simAeroMoveAsset", "HandleMoveAsset", request_json);
}

std::string WorldSimApi::simAeroRemoveAsset(const std::string& request_json)
{
    return invokeAeroBridge("simAeroRemoveAsset", "HandleRemoveAsset", request_json);
}

std::string WorldSimApi::simAeroCaptureWorldCamera(const std::string& request_json)
{
    return invokeAeroBridge("simAeroCaptureWorldCamera", "HandleCaptureWorldCamera", request_json);
}

std::string WorldSimApi::simAeroReserveOccupancy(const std::string& request_json)
{
    return invokeAeroBridge("simAeroReserveOccupancy", "HandleReserveOccupancy", request_json);
}

std::string WorldSimApi::simAeroReleaseOccupancy(const std::string& request_json)
{
    return invokeAeroBridge("simAeroReleaseOccupancy", "HandleReleaseOccupancy", request_json);
}

std::string WorldSimApi::simAeroQueryNearest(const std::string& request_json)
{
    return invokeAeroBridge("simAeroQueryNearest", "HandleQueryNearest", request_json);
}

std::string WorldSimApi::simAeroQueryPedPath(const std::string& request_json)
{
    return invokeAeroBridge("simAeroQueryPedPath", "HandleQueryPedPath", request_json);
}

std::string WorldSimApi::simAeroProjectGround(const std::string& request_json)
{
    return invokeAeroBridge("simAeroProjectGround", "HandleProjectGround", request_json);
}

std::string WorldSimApi::simAeroQueryPedAnchor(const std::string& request_json)
{
    return invokeAeroBridge("simAeroQueryPedAnchor", "HandleQueryPedAnchor", request_json);
}

std::string WorldSimApi::simAeroApplyWeather(const std::string& request_json)
{
...
msr::airlib::CameraInfo WorldSimApi::getCameraInfo(const CameraDetails& camera_details) const
{
    ...
    const APIPCamera* camera = ResolveRpcCamera(simmode_, camera_details);
...
{
    APIPCamera* camera = const_cast<APIPCamera*>(ResolveRpcCamera(simmode_, camera_details));
    ...
    const std::vector<ImageCaptureBase::ImageRequest>& requests, const std::string& vehicle_name) const
{
    ...
    common_utils::UniqueValueMap<std::string, APIPCamera*> request_cameras;
    for (const auto& request : requests) {
        APIPCamera* camera = const_cast<APIPCamera*>(ResolveRpcCamera(simmode_, CameraDetails(request.camera_name, vehicle_name)));
        if (camera == nullptr) {
            for (const auto& failed_request : requests) {
                ImageCaptureBase::ImageResponse response;
                response.camera_name = failed_request.camera_name;
                response.message = "camera is not set";
                responses.push_back(response);
            }
            return responses;
        }
        request_cameras.insert_or_assign(request.camera_name, camera);
    }

    UnrealImageCapture capture(&request_cameras);
    capture.getImages(requests, responses);
...
{
    return ResolveRpcCamera(simmode_, camera_details)->getPresetLensSettings();
}

std::string WorldSimApi::getLensSettings(const CameraDetails& camera_details)
{
    return ResolveRpcCamera(simmode_, camera_details)->getLensSettings();
}

void WorldSimApi::setPresetLensSettings(std::string preset, const CameraDetails& camera_details)
{
    return const_cast<APIPCamera*>(ResolveRpcCamera(simmode_, camera_details))->setPresetLensSettings(preset);
}

std::vector<std::string> WorldSimApi::getPresetFilmbackSettings(const CameraDetails& camera_details)
{
    return ResolveRpcCamera(simmode_, camera_details)->getPresetFilmbackSettings();
}

void WorldSimApi::setPresetFilmbackSettings(std::string preset, const CameraDetails& camera_details)
{
    return const_cast<APIPCamera*>(ResolveRpcCamera(simmode_, camera_details))->setPresetFilmbackSettings(preset);
}

std::string WorldSimApi::getFilmbackSettings(const CameraDetails& camera_details)
{
    return ResolveRpcCamera(simmode_, camera_details)->getFilmbackSettings();
}

float WorldSimApi::setFilmbackSettings(float width, float height, const CameraDetails& camera_details)
{
    return const_cast<APIPCamera*>(ResolveRpcCamera(simmode_, camera_details))->setFilmbackSettings(width, height);
}

float WorldSimApi::getFocalLength(const CameraDetails& camera_details)
{
    return ResolveRpcCamera(simmode_, camera_details)->getFocalLength();
}

void WorldSimApi::setFocalLength(float focal_length, const CameraDetails& camera_details)
{
    return const_cast<APIPCamera*>(ResolveRpcCamera(simmode_, camera_details))->setFocalLength(focal_length);
}

void WorldSimApi::enableManualFocus(bool enable, const CameraDetails& camera_details)
{
    return const_cast<APIPCamera*>(ResolveRpcCamera(simmode_, camera_details))->enableManualFocus(enable);
}

float WorldSimApi::getFocusDistance(const CameraDetails& camera_details)
{
    return ResolveRpcCamera(simmode_, camera_details)->getFocusDistance();
}

void WorldSimApi::setFocusDistance(float focus_distance, const CameraDetails& camera_details)
{
    return const_cast<APIPCamera*>(ResolveRpcCamera(simmode_, camera_details))->setFocusDistance(focus_distance);
}

float WorldSimApi::getFocusAperture(const CameraDetails& camera_details)
{
    return ResolveRpcCamera(simmode_, camera_details)->getFocusAperture();
}

void WorldSimApi::setFocusAperture(float focus_aperture, const CameraDetails& camera_details)
{
    return const_cast<APIPCamera*>(ResolveRpcCamera(simmode_, camera_details))->setFocusAperture(focus_aperture);
}

void WorldSimApi::enableFocusPlane(bool enable, const CameraDetails& camera_details)
{
    return const_cast<APIPCamera*>(ResolveRpcCamera(simmode_, camera_details))->enableFocusPlane(enable);
}

std::string WorldSimApi::getCurrentFieldOfView(const CameraDetails& camera_details)
{
    return ResolveRpcCamera(simmode_, camera_details)->getCurrentFieldOfView();
}
//End CinemAirSim
