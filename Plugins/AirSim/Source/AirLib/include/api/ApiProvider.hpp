...
namespace airlib
{
    ...
    public:
        ...
        void removeVehicle(const std::string& vehicle_name)
        {
            VehicleApiBase* vehicle_api = getVehicleApi(vehicle_name);
            VehicleSimApiBase* vehicle_sim_api = getVehicleSimApi(vehicle_name);

            vehicle_apis_.erase(vehicle_name);
            vehicle_sim_apis_.erase(vehicle_name);

            if (vehicle_name != "") {
                if (vehicle_api != nullptr && vehicle_apis_.findOrDefault("", nullptr) == vehicle_api)
                    vehicle_apis_.erase("");

                if (vehicle_sim_api != nullptr && vehicle_sim_apis_.findOrDefault("", nullptr) == vehicle_sim_api)
                    vehicle_sim_apis_.erase("");
            }
        }
