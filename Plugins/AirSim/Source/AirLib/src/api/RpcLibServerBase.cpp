...
namespace airlib
{
        ...
        : api_provider_(api_provider)
    {
        ...
        pimpl_->server.bind("simAeroDescribeCapabilities", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroDescribeCapabilities(request_json);
        });

        pimpl_->server.bind("simAeroLoadContext", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroLoadContext(request_json);
        });

        pimpl_->server.bind("simAeroReloadConfig", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroReloadConfig(request_json);
        });

        pimpl_->server.bind("simAeroApplyFrame", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroApplyFrame(request_json);
        });

        pimpl_->server.bind("simAeroPollFeedback", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroPollFeedback(request_json);
        });

        pimpl_->server.bind("simAeroPedSpawn", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroPedSpawn(request_json);
        });

        pimpl_->server.bind("simAeroPedReset", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroPedReset(request_json);
        });

        pimpl_->server.bind("simAeroPedSetTarget", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroPedSetTarget(request_json);
        });

        pimpl_->server.bind("simAeroPedObserve", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroPedObserve(request_json);
        });

        pimpl_->server.bind("simAeroPedPlayAnimation", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroPedPlayAnimation(request_json);
        });

        pimpl_->server.bind("simAeroPedCommitCross", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroPedCommitCross(request_json);
        });

        pimpl_->server.bind("simAeroPedStop", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroPedStop(request_json);
        });

        pimpl_->server.bind("simAeroPedSetVariant", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroPedSetVariant(request_json);
        });

        pimpl_->server.bind("simAeroPedRelease", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroPedRelease(request_json);
        });

        pimpl_->server.bind("simAeroPedSpawnCrowd", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroPedSpawnCrowd(request_json);
        });

        pimpl_->server.bind("simAeroPedClearCrowd", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroPedClearCrowd(request_json);
        });

        pimpl_->server.bind("simAeroPedRespawnCrowd", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroPedRespawnCrowd(request_json);
        });

        pimpl_->server.bind("simAeroSpawnAsset", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroSpawnAsset(request_json);
        });

        pimpl_->server.bind("simAeroMoveAsset", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroMoveAsset(request_json);
        });

        pimpl_->server.bind("simAeroRemoveAsset", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroRemoveAsset(request_json);
        });

        pimpl_->server.bind("simAeroCaptureWorldCamera", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroCaptureWorldCamera(request_json);
        });

        pimpl_->server.bind("simAeroReserveOccupancy", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroReserveOccupancy(request_json);
        });

        pimpl_->server.bind("simAeroReleaseOccupancy", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroReleaseOccupancy(request_json);
        });

        pimpl_->server.bind("simAeroQueryNearest", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroQueryNearest(request_json);
        });

        pimpl_->server.bind("simAeroQueryPedPath", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroQueryPedPath(request_json);
        });

        pimpl_->server.bind("simAeroProjectGround", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroProjectGround(request_json);
        });

        pimpl_->server.bind("simAeroQueryPedAnchor", [&](const std::string& request_json) -> std::string {
            return getWorldSimApi()->simAeroQueryPedAnchor(request_json);
        });

        pimpl_->server.bind("simAeroApplyWeather", [&](const std::string& request_json) -> std::string {
