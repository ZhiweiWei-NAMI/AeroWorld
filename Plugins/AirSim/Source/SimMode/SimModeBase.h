...
public:
    ...
    bool removeVehicleAtRuntime(const std::string& vehicle_name);
    ...
    APawn* resolveVehiclePawn(const std::string& vehicle_name) const;
...
protected: //must overrides
    ...
    virtual void removePhysicsBody(msr::airlib::VehicleSimApiBase* physicsBody);

protected: //optional overrides
    virtual APawn* createVehiclePawn(const AirSimSettings::VehicleSetting& vehicle_setting);
    virtual std::unique_ptr<PawnSimApi> createVehicleApi(APawn* vehicle_pawn, const std::string& vehicle_name_override = "");
