...
namespace
{
constexpr float AirSimSpawnGroundTraceHalfHeightCm = 250000.0f;
constexpr float AirSimDefaultGroundVehicleClearanceCm = 20.0f;
constexpr float AirSimDefaultMultirotorClearanceCm = 300.0f;
...
bool IsPreferredGroundActor(const AActor* Actor)
{
    ...
    return ActorName.Contains(TEXT("road"), ESearchCase::IgnoreCase) ||
...
bool TryProjectSpawnPointToGround(UWorld* World, const FVector& RequestedWorldCm, FVector& OutProjectedWorldCm, FVector& OutSurfaceNormalWorld)
{
    OutProjectedWorldCm = RequestedWorldCm;
    OutSurfaceNormalWorld = FVector::UpVector;
    if (World == nullptr) {
        return false;
    }

    const FVector TraceStart = RequestedWorldCm + FVector(0.0f, 0.0f, AirSimSpawnGroundTraceHalfHeightCm);
    const FVector TraceEnd = RequestedWorldCm - FVector(0.0f, 0.0f, AirSimSpawnGroundTraceHalfHeightCm);
    TArray<FHitResult> HitResults;
    FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(AirSimSpawnGroundProjection), true);
    ...
    OutProjectedWorldCm = SelectedHit->ImpactPoint;
...
bool IsMultirotorVehicleType(const std::string& VehicleType)
{
    return VehicleType == msr::airlib::AirSimSettings::kVehicleTypeSimpleFlight ||
           VehicleType == msr::airlib::AirSimSettings::kVehicleTypePX4 ||
           VehicleType == msr::airlib::AirSimSettings::kVehicleTypeArduCopterSolo ||
           VehicleType == msr::airlib::AirSimSettings::kVehicleTypeArduCopter;
}

bool TryResolveRoadAnchorForDefaultSpawn(
    UWorld* World,
    const FVector& PrimaryQueryWorldCm,
    FSumoNearestLaneSample& OutRoadSample,
    FString& OutError)
{
    ...
    TArray<FVector> QuerySeeds;
    QuerySeeds.Add(PrimaryQueryWorldCm);
    if (APlayerController* PlayerController = World->GetFirstPlayerController()) {
        if (AActor* ViewTarget = PlayerController->GetViewTarget()) {
            QuerySeeds.AddUnique(ViewTarget->GetActorLocation());
        }
        else if (APawn* Pawn = PlayerController->GetPawn()) {
            QuerySeeds.AddUnique(Pawn->GetActorLocation());
        }
    }

    FString LastError;
    for (const FVector& QuerySeed : QuerySeeds) {
        FSumoNearestLaneSample CandidateSample;
        FString CandidateError;
        if (FSumoRoadTopologyQuery::FindNearestRoadSample(World, QuerySeed, CandidateSample, CandidateError)) {
            OutRoadSample = CandidateSample;
            return true;
        }
    ...
    OutError = LastError.IsEmpty()
                   ? TEXT("No SUMO road topology anchor was found for default AirSim vehicle spawn.")
                   : LastError;
    return false;
}

bool LooksLikeDirectClassPath(const std::string& Path)
{
    return Path.rfind("Class'", 0) == 0 || Path.rfind("/Game/", 0) == 0 ||
           Path.rfind("/AirSim/", 0) == 0 || Path.rfind("/AeroWorldContent/", 0) == 0;
}

UClass* TryLoadPawnClassByPath(const std::string& ClassPath)
{
    if (ClassPath.empty()) {
        return nullptr;
    }

    const FString ClassPathString(ClassPath.c_str());
    return StaticLoadClass(UObject::StaticClass(), nullptr, *ClassPathString);
}
} // namespace
...
void ASimModeBase::initializeCameraDirector(const FTransform& camera_transform, float follow_distance)
{
    ...
    if (CameraDirector != nullptr && CameraDirector->ExternalCamera != nullptr) {
        auto external_camera_setting = getSettings().camera_defaults;
        external_camera_setting.external = true;
        CameraDirector->ExternalCamera->setupCameraFromSettings(external_camera_setting, getGlobalNedTransform());
    }
}
...
APawn* ASimModeBase::resolveVehiclePawn(const std::string& vehicle_name) const
{
    ...
    return const_cast<PawnSimApi*>(vehicle_sim_api)->getPawn();
}
...
APawn* ASimModeBase::createVehiclePawn(const AirSimSettings::VehicleSetting& vehicle_setting)
{
    ...
    const bool bHasExplicitSpawnPosition = !VectorMath::hasNan(settings_position);
    FRotator default_spawn_rotation = uu_origin.Rotator();
    if (bHasExplicitSpawnPosition)
        spawn_position = getGlobalNedTransform().fromGlobalNed(settings_position);
    else {
        FSumoNearestLaneSample RoadSample;
        FString RoadAnchorError;
        if (!TryResolveRoadAnchorForDefaultSpawn(GetWorld(), spawn_position, RoadSample, RoadAnchorError)) {
            UE_LOG(LogTemp,
                   Error,
                   TEXT("AirSim skipped auto-create for vehicle '%s': missing road topology anchor. %s"),
                   *FString(vehicle_setting.vehicle_name.c_str()),
                   *RoadAnchorError);
            return nullptr;
        }

        const float clearance_cm = IsMultirotorVehicleType(vehicle_setting.vehicle_type)
                                       ? AirSimDefaultMultirotorClearanceCm
                                       : AirSimDefaultGroundVehicleClearanceCm;
        spawn_position = RoadSample.WorldTransform.GetLocation() + FVector::UpVector * clearance_cm;
        default_spawn_rotation = FRotator(0.0f, RoadSample.WorldTransform.Rotator().Yaw, 0.0f);
        UE_LOG(LogTemp,
               Log,
               TEXT("AirSim default vehicle '%s' had no explicit spawn pose; anchored to lane='%s' edge='%s' sample='%s' clearance=%.1fcm final='%s' yaw=%.2f."),
               *FString(vehicle_setting.vehicle_name.c_str()),
               *RoadSample.LaneId,
               *RoadSample.EdgeId,
               *RoadSample.WorldTransform.GetLocation().ToString(),
               clearance_cm,
               *spawn_position.ToString(),
               default_spawn_rotation.Yaw);
    }

    FRotator spawn_rotation = toFRotator(vehicle_setting.rotation, default_spawn_rotation);

    std::string vehicle_name = vehicle_setting.vehicle_name;
    const std::string pawn_path_name = getVehiclePawnPathName(vehicle_setting);

    //spawn vehicle pawn
    FActorSpawnParameters pawn_spawn_params;
    pawn_spawn_params.Name = FName(vehicle_name.c_str());
    pawn_spawn_params.NameMode = FActorSpawnParameters::ESpawnActorNameMode::Requested;
    pawn_spawn_params.SpawnCollisionHandlingOverride =
        ESpawnActorCollisionHandlingMethod::AdjustIfPossibleButAlwaysSpawn;

    UClass* vehicle_bp_class = nullptr;
    const auto pawn_path_it = getSettings().pawn_paths.find(pawn_path_name);
    if (pawn_path_it != getSettings().pawn_paths.end()) {
        vehicle_bp_class = TryLoadPawnClassByPath(pawn_path_it->second.pawn_bp);
        if (vehicle_bp_class == nullptr) {
            UE_LOG(LogTemp,
                   Error,
                   TEXT("AirSim failed to load pawn class path '%s' for vehicle '%s' (pawn key '%s')."),
                   *FString(pawn_path_it->second.pawn_bp.c_str()),
                   *FString(vehicle_name.c_str()),
                   *FString(pawn_path_name.c_str()));
            return nullptr;
        }
    }
    else if (LooksLikeDirectClassPath(pawn_path_name)) {
        vehicle_bp_class = TryLoadPawnClassByPath(pawn_path_name);
        if (vehicle_bp_class == nullptr) {
            UE_LOG(LogTemp,
                   Error,
                   TEXT("AirSim failed to load direct pawn class path '%s' for vehicle '%s'."),
                   *FString(pawn_path_name.c_str()),
                   *FString(vehicle_name.c_str()));
            return nullptr;
        }
        ...
        UE_LOG(LogTemp,
               Error,
               TEXT("AirSim pawn path '%s' was not registered for vehicle '%s'."),
               *FString(pawn_path_name.c_str()),
               *FString(vehicle_name.c_str()));
        return nullptr;
    }
    ...
    if (spawned_pawn == nullptr) {
        UE_LOG(LogTemp,
               Error,
               TEXT("AirSim failed to spawn pawn for vehicle '%s' using class '%s'."),
               *FString(vehicle_name.c_str()),
               vehicle_bp_class != nullptr ? *vehicle_bp_class->GetName() : TEXT("<null>"));
        return nullptr;
    }

    if (!spawned_pawn->GetName().Equals(FString(vehicle_name.c_str()), ESearchCase::CaseSensitive)) {
        UE_LOG(LogTemp,
               Warning,
               TEXT("AirSim runtime vehicle '%s' spawned with unique actor name '%s' due to an existing object name collision."),
               *FString(vehicle_name.c_str()),
               *spawned_pawn->GetName());
    }
...
std::unique_ptr<PawnSimApi> ASimModeBase::createVehicleApi(APawn* vehicle_pawn, const std::string& vehicle_name_override)
{
    if (vehicle_pawn == nullptr) {
        UE_LOG(LogTemp, Error, TEXT("AirSim createVehicleApi received a null pawn."));
        return nullptr;
    }
    ...
    const std::string vehicle_name = !vehicle_name_override.empty()
        ? vehicle_name_override
        : std::string(TCHAR_TO_UTF8(*(vehicle_pawn->GetName())));
                                          ...
                                          const msr::airlib::Pose& pose, const std::string& pawn_path)
{
    ...
    if (vehicle_setting == nullptr) {
        UE_LOG(LogTemp, Error, TEXT("AirSim failed to resolve runtime vehicle settings for '%s'."), *FString(vehicle_name.c_str()));
        return false;
    }

    auto spawned_pawn = createVehiclePawn(*vehicle_setting);
    if (spawned_pawn == nullptr) {
        UE_LOG(LogTemp, Error, TEXT("AirSim failed to create runtime pawn for vehicle '%s'."), *FString(vehicle_name.c_str()));
        return false;
    }

    auto vehicle_sim_api = createVehicleApi(spawned_pawn, vehicle_name);
    if (!vehicle_sim_api) {
        UE_LOG(LogTemp, Error, TEXT("AirSim failed to create runtime vehicle API for '%s'."), *FString(vehicle_name.c_str()));
        return false;
    }
...
bool ASimModeBase::removeVehicleAtRuntime(const std::string& vehicle_name)
{
    if (vehicle_name.empty() || getApiProvider() == nullptr)
        return false;

    msr::airlib::VehicleApiBase* vehicle_api = getApiProvider()->getVehicleApi(vehicle_name);
    auto* vehicle_sim_api = getApiProvider()->getVehicleSimApi(vehicle_name);
    auto* pawn_sim_api = static_cast<PawnSimApi*>(vehicle_sim_api);
    APawn* vehicle_pawn = pawn_sim_api != nullptr ? pawn_sim_api->getPawn() : nullptr;

    if (vehicle_api == nullptr && vehicle_sim_api == nullptr && vehicle_pawn == nullptr) {
        for (TActorIterator<APawn> It(GetWorld()); It; ++It) {
            APawn* CandidatePawn = *It;
            if (IsValid(CandidatePawn) && CandidatePawn->GetName().Equals(FString(vehicle_name.c_str()), ESearchCase::CaseSensitive)) {
                CandidatePawn->Destroy();
                return true;
            }
        }
        return false;
    ...
    getApiProvider()->removeVehicle(vehicle_name);

    if (vehicle_sim_api != nullptr) {
        removePhysicsBody(vehicle_sim_api);

        const auto vehicle_sim_api_it = std::find_if(vehicle_sim_apis_.begin(),
                                                     vehicle_sim_apis_.end(),
                                                     [vehicle_sim_api](const std::unique_ptr<msr::airlib::VehicleSimApiBase>& owned_api) {
                                                         return owned_api.get() == vehicle_sim_api;
                                                     });
        if (vehicle_sim_api_it != vehicle_sim_apis_.end())
            vehicle_sim_apis_.erase(vehicle_sim_api_it);
    }

    if (vehicle_pawn != nullptr) {
        spawned_actors_.Remove(vehicle_pawn);
        if (IsValid(vehicle_pawn))
            vehicle_pawn->Destroy();
    }

    spawned_actors_.RemoveAll([](const AActor* actor) { return !IsValid(actor); });
...
void ASimModeBase::setupVehiclesAndCamera()
{
    ...
    //find all vehicle pawns
    {
            ...
            for (const auto& vehicle_setting_pair : getSettings().vehicles) {
                ...
                if (vehicle_setting.auto_create &&
                    ...
                    if (spawned_pawn == nullptr) {
                        UE_LOG(LogTemp,
                               Error,
                               TEXT("AirSim failed to auto-create pawn for vehicle '%s'; skipping API initialization."),
                               *FString(vehicle_setting.vehicle_name.c_str()));
                        continue;
                    }
        ...
        for (AActor* pawn : pawns) {
            ...
            if (vehicle_pawn == nullptr) {
                UE_LOG(LogTemp, Warning, TEXT("AirSim encountered a null pawn while initializing vehicle APIs; skipping."));
                continue;
            }
            ...
            if (!vehicle_sim_api) {
                UE_LOG(LogTemp,
                       Error,
                       TEXT("AirSim failed to create vehicle API for pawn '%s'; skipping default-vehicle registration."),
                       *vehicle_pawn->GetName());
                continue;
            }
            std::string vehicle_name = vehicle_sim_api->getVehicleName();
...
void ASimModeBase::removePhysicsBody(msr::airlib::VehicleSimApiBase* physicsBody)
{
    unused(physicsBody);
    // derived class should override this method to remove vehicle from the physics engine
}
