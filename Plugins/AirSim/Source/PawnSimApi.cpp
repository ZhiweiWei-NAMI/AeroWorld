...
namespace
{
float SanitizeRotationComponent(const float Value)
{
    return std::isnan(Value) ? 0.0f : Value;
}

bool HasInvalidRotation(const msr::airlib::AirSimSettings::Rotation& Rotation)
{
    return std::isnan(Rotation.pitch) || std::isnan(Rotation.yaw) || std::isnan(Rotation.roll);
}

FRotator MakeSafeRotator(const msr::airlib::AirSimSettings::Rotation& Rotation)
{
    return FRotator(
        SanitizeRotationComponent(Rotation.pitch),
        SanitizeRotationComponent(Rotation.yaw),
        SanitizeRotationComponent(Rotation.roll));
}
} // namespace
...
void PawnSimApi::createCamerasFromSettings()
{
    ...
    for (const auto& camera_setting_pair : getVehicleSetting()->cameras) {
        ...
        if (HasInvalidRotation(setting.rotation)) {
            UAirBlueprintLib::LogMessageString(
                "AirSim camera rotation contained NaN; defaulting to zero for camera",
                camera_setting_pair.first,
                LogDebugLevel::Failure);
        }
        FTransform camera_transform(MakeSafeRotator(setting.rotation),
                                    position,
                                    FVector(1., 1., 1.));
...
void PawnSimApi::createAndInitializeFromSettings()
{
    ...
    for (const auto& light_setting_pair : getVehicleSetting()->lights) {
        ...
        if (HasInvalidRotation(setting.rotation)) {
            UAirBlueprintLib::LogMessageString(
                "AirSim light rotation contained NaN; defaulting to zero for light",
                setting.name,
                LogDebugLevel::Failure);
        }
        FRotator rotation = MakeSafeRotator(setting.rotation);
