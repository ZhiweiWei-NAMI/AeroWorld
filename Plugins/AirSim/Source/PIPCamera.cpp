...
namespace
{
float PIPCameraSanitizeRotationComponent(const float Value)
{
    return std::isnan(Value) ? 0.0f : Value;
}

bool PIPCameraHasInvalidRotation(const msr::airlib::AirSimSettings::Rotation& Rotation)
{
    return std::isnan(Rotation.pitch) || std::isnan(Rotation.roll) || std::isnan(Rotation.yaw);
}
} // namespace
...
{
    if (ned_transform_ == nullptr) {
        UAirBlueprintLib::LogMessageString(
            "APIPCamera::setCameraPose skipped because ned_transform_ is not initialized",
            std::string(TCHAR_TO_UTF8(*GetName())),
            LogDebugLevel::Failure);
        return;
    }
...
msr::airlib::CameraInfo APIPCamera::getCameraInfo() const
{
    ...
    if (ned_transform_ == nullptr || camera_ == nullptr) {
        camera_info.pose = msr::airlib::Pose::nanPose();
        camera_info.fov = Utils::nan<float>();
...
void APIPCamera::setupCameraFromSettings(const APIPCamera::CameraSetting& camera_setting, const NedTransform& ned_transform)
{
    ...
    if (gimbal_stabilization_ > 0) {
        ...
        if (PIPCameraHasInvalidRotation(camera_setting.gimbal.rotation)) {
            UAirBlueprintLib::LogMessageString(
                "AirSim camera gimbal rotation contained NaN; defaulting to zero",
                "",
                LogDebugLevel::Failure);
        }
        gimbald_rotator_.Pitch = PIPCameraSanitizeRotationComponent(camera_setting.gimbal.rotation.pitch);
        gimbald_rotator_.Roll = PIPCameraSanitizeRotationComponent(camera_setting.gimbal.rotation.roll);
        gimbald_rotator_.Yaw = PIPCameraSanitizeRotationComponent(camera_setting.gimbal.rotation.yaw);
    }
    else
        this->SetActorTickEnabled(false);
